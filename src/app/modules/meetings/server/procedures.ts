...
export const meetingsRouter = createTRPCRouter({
    genrateChatToken: protectedProcedure
        .mutation( async ({ctx})=>{
                ...
                const userSpeakers = await db
                    ...
                    .then((users)=>
                        users.map((user)=>({
                            ...
                            image:
                                user.image ??
                                genrateAvatarUri({seed: user.name, variant:"initials"})
                        }))
                    )
                const agentSpeakers = await db
                    ...
                    .then((agents)=>
                        agents.map((agent)=>({
                            ...agent,
                            image:
                                genrateAvatarUri({seed: agent.name, variant:"botttsNeutral"})
                        }))
                    )
                ...
                const transcriptWithSpeakers = transcript.map((item)=>{
                    ...
                    if(!speaker) {
                        return{
                            ...
                            user: {
                                name: "Unknown",
                                image: genrateAvatarUri({seed: "Unknown", variant:"initials"})
                            }
                        }
                    }
    ...
    genrateToken: protectedProcedure
            .mutation( async ({ctx})=>{
                // user created for call
                await streamVideo.upsertUsers([
                    {
                        id: ctx.auth.user.id,
                        name: ctx.auth.user.name,
                        role: "admin",
                        image: ctx.auth.user.image ?? genrateAvatarUri({seed: ctx.auth.user.name, variant:"initials"})
                    }
                ])
                // token
                ...
                // "generateUserToken" => returns no promise , so no await
                    ...
                    await streamVideo.upsertUsers([
                        {
                            ...
                            role: "user",
                            image: genrateAvatarUri({seed: existingAgent.name, variant:"botttsNeutral"})
                        }
                    ])
